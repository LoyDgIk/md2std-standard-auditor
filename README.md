...
- `{{std:...}}` 与“规范性引用文件”章显式注册条目的匹配关系；推荐清单写法为 `{{std:GB/T 11615}} GB/T 11615  地热资源地质勘查规范`，旧版自动识别条目仅作为兼容回退并报告 warning。
- 当前 md2std Markdown 契约：显式术语块、无标题条、组合图题、图表通用附加项、图标引、图段、分图组、表内注和表内脚注引用。
- 已移除的旧式图表附加项和旧分图语法。
- `{无标题条:n}` 是否误用于范围、规范性引用文件、术语和定义、符号和缩略语等基础章。
- 图表通用附加项是否紧跟合法表格或图片目标，以及 `{单位}`、`{来源}` 是否重复。
- `# 范围` 是否存在并位于首个一级章。
- 附录、参考文献、索引的基本顺序。
- 附录中是否错误设置“范围”“规范性引用文件”“术语和定义”等章。
- 规范性引用文件章是否错误分条、使用列表符号，是否存在“无引用”说明与引用清单冲突；省略固定导语时给出生成器默认导语提示。
- 规范性引用文件清单顺序是否疑似违反标准类型和编号排序规则。
- 术语和定义章中“无术语”说明是否与术语条目冲突，或固定导语是否与实际条目不匹配。
- 术语和定义章使用生成器默认导语时，提示如同时引用外部术语应手写组合导语。
- 符号和缩略语章是否为空章。
- 手写 GB/T 1.1 起草依据语句是否疑似偏离固定表述。
